...
namespace zCode.zMesh
{
    ...
    public static class HeStructureExtensions
    {
        ...
        /// <param name="graph"></param>
        /// <returns></returns>
        public static IEnumerable<E> GetCornerHalfedges<V, E, F>(this HeStructure<V, E, F> mesh, Func<V, Vec3d> getPosition, double angleTolerance = zMath.ZeroTolerance)
            where V : HeVertex<V, E, F>
            where E : Halfedge<V, E, F>
            where F : HeFace<V, E, F>
        {
            ...
            void Body(int from, int to)
            {
                for (int i = from; i < to; i++)
                {
                    ...
                    else
                    {
                        ...
                        setCurvature(v, (zMath.TwoPI - sum) / getArea(v));
                    }
                }
            }
        }
