...
namespace zCode.zMesh
{
    ...
    public static class MeshIO
    {
        ...
        /// <summary>
        /// Json serializable representation of a halfedge graph using Open Mesh half edge format with half edge storing the reference to end vertex in place of start one.
        /// https://www.graphics.rwth-aachen.de/media/openmesh_static/Documentations/OpenMesh-8.0-Documentation/a04074.html
        /// </summary>
        [DataContract]
        private class HeGraphJsonBufferOM<VA, EA>
            {
            [DataMember(Name = "Vertices", Order = 0)]
            private int[] _vertexRefs;
            ...
            public void ReadTo<V, E>(HeGraph<V, E> graph, Action<V, VA> setVertexAttributes = null, Action<E, EA> setHedgeAttributes = null)
                ...
                for (int i = 0; i < _hedgeRefs.Length; i++)
                    ...
                    var end = refs[2];
                    if (end > -1) he.Twin.Start = verts[end + nhe];
        ...
        /// <summary>
        /// Json serializable representation of a halfedge mesh using Open Mesh half edge formatwith half edge storing the reference to end vertex in place of start one.
        /// https://www.graphics.rwth-aachen.de/media/openmesh_static/Documentations/OpenMesh-8.0-Documentation/a04074.html
        /// </summary>
        [DataContract]
        private class HeMeshJsonBufferOM<VA, EA, FA>
            {
            [DataMember(Name = "Vertices", Order = 0)]
            private int[] _vertexRefs;
        ...
        /// <typeparam name="V"></typeparam>
        /// <typeparam name="E"></typeparam>
        public static void WriteToJsonOM<V, E, VA, EA>(HeGraph<V, E> graph, string path, Func<V, VA> getVertexAttributes = null, Func<E, EA> getHedgeAttributes = null)
            where V : HeGraph<V, E>.Vertex
            where E : HeGraph<V, E>.Halfedge
            {
            var buffer = new HeGraphJsonBufferOM<VA, EA>();
            buffer.WriteFrom(graph, getVertexAttributes, getHedgeAttributes);
            CoreIO.SerializeJson(buffer, path);
            }
        ...
        /// <param name="setVertexAttributes"></param>
        /// <param name="setHedgeAttributes"></param>
        public static void ReadFromJsonOM<V, E, VA, EA>(string path, HeGraph<V, E> graph, Action<V, VA> setVertexAttributes = null, Action<E, EA> setHedgeAttributes = null)
            where V : HeGraph<V, E>.Vertex
            where E : HeGraph<V, E>.Halfedge
            {
            var buffer = CoreIO.DeserializeJson<HeGraphJsonBufferOM<VA, EA>>(path);
            buffer.ReadTo(graph, setVertexAttributes, setHedgeAttributes);
            }
        ...
        /// <typeparam name="V"></typeparam>
        /// <typeparam name="E"></typeparam>
        public static void WriteToJsonOM<V, E, F, VA, EA, FA>(HeMesh<V, E, F> mesh, string path, Func<V, VA> getVertexAttributes = null, Func<E, EA> getHedgeAttributes = null, Func<F, FA> getFaceAttributes = null)
            where V : HeMesh<V, E, F>.Vertex
            where E : HeMesh<V, E, F>.Halfedge
            where F : HeMesh<V, E, F>.Face
            {
            var buffer = new HeMeshJsonBufferOM<VA, EA, FA>();
            buffer.WriteFrom(mesh, getVertexAttributes, getHedgeAttributes, getFaceAttributes);
            CoreIO.SerializeJson(buffer, path);
            }
        ...
        /// <param name="setVertexAttributes"></param>
        /// <param name="setHedgeAttributes"></param>
        public static void ReadFromJsonOM<V, E, F, VA, EA, FA>(string path, HeMesh<V, E, F> mesh, Action<V, VA> setVertexAttributes = null, Action<E, EA> setHedgeAttributes = null, Action<F, FA> setFaceAttributes = null)
            where V : HeMesh<V, E, F>.Vertex
            where E : HeMesh<V, E, F>.Halfedge
            where F : HeMesh<V, E, F>.Face
            {
            var buffer = CoreIO.DeserializeJson<HeMeshJsonBufferOM<VA, EA, FA>>(path);
            buffer.ReadTo(mesh, setVertexAttributes, setHedgeAttributes, setFaceAttributes);
        ...
        /// <typeparam name="TV"></typeparam>
        /// <typeparam name="TE"></typeparam>
        public static void WriteToJsonOM(HeGraph3d graph, string path)
            {
            WriteToJsonOM<HeGraph3d.Vertex, HeGraph3d.Halfedge, double[], double[]>(graph, path, ToJson);
        ...
        /// <param name="setVertexAttributes"></param>
        /// <param name="setHedgeAttributes"></param>
        public static void ReadFromJsonOM(string path, HeGraph3d graph)
            {
            ReadFromJsonOM<HeGraph3d.Vertex, HeGraph3d.Halfedge, double[], double[]>(path, graph, FromJson);
        ...
        /// <typeparam name="TV"></typeparam>
        /// <typeparam name="TE"></typeparam>
        public static void WriteToJsonOM(HeMesh3d mesh, string path)
            {
            WriteToJsonOM<HeMesh3d.Vertex, HeMesh3d.Halfedge, HeMesh3d.Face, double[], double[], double[]>(mesh, path, ToJson);
        ...
        /// <param name="setVertexAttributes"></param>
        /// <param name="setHedgeAttributes"></param>
        public static void ReadFromJsonOM(string path, HeMesh3d mesh)
            {
            ReadFromJsonOM<HeMesh3d.Vertex, HeMesh3d.Halfedge, HeMesh3d.Face, double[], double[], double[]>(path, mesh, FromJson);
